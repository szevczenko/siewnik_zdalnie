...
#include "cmd_short.h"
#define INDENT "   "
#define NO_HELP "ERR: No help available." NL
#define NL "\r\n"
...
static char space[] = "               ";
#endif
uint8_t cosole_echo_flag = CONFIG_CONSOLE_ECHO;
...
static int split_line(t_tokenline *tl, int *words, int *num_words, int silent)
{
	...
	*num_words = 0;
	...
	if (quoted) {
		if (!silent)
			tl->print(tl->user, "ERR: Unmatched quote." NL);
		unsplit_line(tl);
		return FALSE;
	}
	if (*num_words == TL_MAX_WORDS) {
		if (!silent)
			tl->print(tl->user, "ERR: Too many words." NL);
		unsplit_line(tl);
		return FALSE;
	}
		...
		t_token **complete_tokens, int *complete_arg)
{
	...
	for (w = 0; w < num_words; w++) {
		...
		if (done) {
			if (!complete_tokens)
				tl->print(tl->user, "ERR: Too many arguments." NL);
			return FALSE;
		} else if (!arg_needed) {
			/* Token needed. */
			...
			if ((t_idx = find_token(token_stack[cur_tsp], tl->token_dict, word)) > -1) {
				...
				if (suffix) {
					if (!(token_stack[cur_tsp][t_idx].flags &
						...
						if (!complete_tokens)
							tl->print(tl->user, "ERR: Token suffix not allowed." NL);
						return FALSE;
					}
					if (suffix_uint > 1) {
				...
				if (token_stack[cur_tsp][i].token) {
					...
					if (!complete_tokens)
						tl->print(tl->user, "ERR: Invalid command." NL);
					return FALSE;
				}
			}
		} else {
			...
			case T_ARG_UINT:
				...
				{
					if (!complete_tokens)
						tl->print(tl->user, "ERR: Invalid value." NL);
					return FALSE;
				}
				if (suffix) {
					switch(*suffix)
...
static void process_line(t_tokenline *tl)
{
	...
	do {
		...
		if (!strcmp(tl->buf + words[0], "help")) {
			...
			#if CONFIG_USE_SLCAN && CONFIG_USE_CONSOLE_CMDSHORT
			if (num_words == 1)
				short_cmd_help(tl);
			#endif //CONFIG_USE_SLCAN && CONFIG_USE_CONSOLE_CMDSHORT
		}
		else if (!strcmp(tl->buf + words[0], "history")) {
			history_show(tl);
		}
		else {
			if (num_words < 2 && processCmdShort(tl))
			#if CONFIG_USE_CONSOLE_TOKEN
				break;
	...
	tl->pos = 0;
	tl->hist_step = -1;
	tl->print(tl->user, tl->prompt); //echo
}
...
static void add_char(t_tokenline *tl, int c)
{
	...
	if (tl->pos == tl->buf_len) {
		...
		if (cosole_echo_flag == 1)
			tl->print(tl->user, tl->buf + tl->buf_len - 1); //echo
		tl->pos++;
	} else {
		...
		tl->buf[tl->pos] = c;
		if (cosole_echo_flag == 1) {
			tl->print(tl->user, tl->buf + tl->pos); //echo
			for (i = 0; i < tl->buf_len - tl->pos; i++)
				tl->print(tl->user, "\x1b\x5b\x44"); //echo
		}
		tl->buf_len++;
		tl->pos++;
	}
...
static void line_backspace(t_tokenline *tl)
{
	...
	if (tl->pos == tl->buf_len) {
		...
		if (cosole_echo_flag == 1) {
			tl->print(tl->user, "\x1b\x5b\x44 \x1b\x5b\x44"); //echo
		}
	} else {
		memmove(tl->buf + tl->pos - 1, tl->buf + tl->pos,
				tl->buf_len - tl->pos + 1);
